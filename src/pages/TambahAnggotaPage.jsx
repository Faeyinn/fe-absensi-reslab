...
export default function TambahAnggota() {
    ...
    const validateForm = () => {
        ...
        } else if (dataStore.isNimExists(nimValue)) {
        ...
        } else if (dataStore.isRfidExists(rfidValue)) {
            newErrors.idRfid = 'ID RFID sudah terdaftar';
        }
    ...
    const handleSave = () => {
        ...
        setTimeout(() => {
            try {
                ...
                showNotification(`Anggota "${newMember.nama}" berhasil ditambahkan!`, 'success');
                
                setTimeout(() => {
                    navigate('/anggota');
                }, 1500);
            } catch (error) {
                showNotification('Gagal menambahkan anggota', error.message || 'error');
            } finally {
                setIsLoading(false);
            }
        }, 1000);
