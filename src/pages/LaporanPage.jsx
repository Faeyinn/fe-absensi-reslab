...
export default function LaporanPage() {
    ...
    useEffect(() => {
        const updateData = () => {
            const data = dataStore.getAttendance();
    ...
    const generateCSV = (data) => {
        const headers = ["No", "Nama", "Tanggal", "Jam", "Status"];
    ...
    const generateTextReport = (data) => {
        ...
        report += `${'No'.padEnd(4)} ${'Nama'.padEnd(25)} ${'Tanggal'.padEnd(12)} ${'Jam'.padEnd(8)} Status\n`;
        report += `${'-'.repeat(60)}\n`;
        
        data.forEach((item, index) => {
            report += `${(index + 1).toString().padEnd(4)} ${item.nama.padEnd(25)} ${item.tanggal.padEnd(12)} ${item.jam.padEnd(8)} ${item.status}\n`;
        });
